...
export default function JanelaEntidades({entidade, fecharJanela}: janelaProps) {
    const nodeRef = useRef(null)
    return(
    <Draggable nodeRef={nodeRef} handle='.barra' bounds="body">
    <div ref={nodeRef} className='w-170 h-fit flex flex-col z-100 fixed top-1/2 left-1/3'>
        ...
        <div style={{borderColor: entidade.cor}} className={`border-8 border-t-0 h-full px-6 py-8 flex gap-8 w-full bg-black`}>
            <div className='flex flex-col gap-5 items-center w-fit'>
                <Image src={entidade.image} width={160} height={180} alt='Logo do PET Informática'/>
